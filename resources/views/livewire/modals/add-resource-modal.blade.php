        ...
        <div class="modal-body">
            ...
            <table class="table table-bordered table-striped" id="resource-table">
                ...
                <tr class="prodheader">
                    ...
                    @if($resource_type == 'Product') <th>@lang('bt.product_numstock')</th> @endif
                    <th>@lang('bt.price')</th>
                </tr>
                </thead>
                <tbody>
                @foreach ($resources as $resource)
                    ...
                    <tr class="prodlist">
                        ...
                        <td>{!!  $resource->name ?? $resource->short_name !!}</td>
                        <td>{!!  $resource->description ?? $resource->title !!}</td>
                        @if($resource_type == 'Product') <td @if($resource->is_trackable) class="bg-secondary" title="@lang('bt.trackable')" @endif>{!!  \BT\Support\NumberFormatter::format($resource->numstock,null,2) ?? 0 !!} @endif
                        <td>{!!  $resource->formatted_cost ?? $resource->formatted_price ?? $resource->formatted_billing_rate !!}</td>
                    </tr>
                    @endif
                @endforeach
